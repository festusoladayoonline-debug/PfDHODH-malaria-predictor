...
targeting Plasmodium falciparum (PfDHODH) using machine learning models (regression and classification) trained on ChEMBL bioactivity data.

Author: Festus Ogungbemiro - Computational Drug Discovery Pipeline for CADD 2.0 Tutorial
Date: April, 2026
"""
...
st.set_page_config(
    page_title="Dihydroorotate dehydrogenase inhibitors of Plasmodium falciparum (PfDHODH)Bioactivity Predictor",
    page_icon="🧬",
    layout="wide",
...
def load_models():
    ...
    try:
        ...
        st.info("Please ensure the 'PfDHODH_deployment_models' directory exists with all pickle files.")
        return None
...
def classify_bioactivity(pic50):
    """Classify compound as Active/Intermediate/Inactive based on pIC50."""
    if pic50 >= 6.0:
        return 'Active', '🟢'
    elif 5.0 < pic50 < 6.0:
        return 'Intermediate', '🟡'
...
def main():
    ...
    if models is None:
        ...
        return
    # Header
    st.markdown("<div class='main-header'>🧬 Dihydroorotate dehydrogenase inhibitors of Plasmodium falciparum (PfDHODH) Bioactivity Predictor</div>", unsafe_allow_html=True)
    st.markdown("<div class='sub-header'>Predict pIC50 values for PfDHODH inhibitors using machine learning</div>", unsafe_allow_html=True)
    
    # Display a scientific image related to PfDHODH
    col_img1, col_img2, col_img3 = st.columns([1, 2, 1])
    with col_img2:
        st.image("assets/pfdhodh_structure.jpg", caption="Protein Structure of PfDHODH (Target for Antimalarials)", use_container_width=True)
                ...
                if mol is None:
                    ...
                    with st.spinner("Calculating molecular descriptors and making predictions..."):
                        ...
                        if predictions is None:
                            ...
                            # Model comparison
                            st.markdown("### 🤖 PfDHODH Model Comparison")
            ...
            try:
                ...
                if 'SMILES' not in df.columns:
                    ...
                    st.download_button(
                        ...
                        data=csv,
                        file_name="PfDHODH_predictions.csv",
                        mime="text/csv"
                    )
    ...
    # ========================================================================
        ...
        ** Plasmodium falciparum dihydroorotate dehydrogenase (PfDHODH) (G-protein coupled      
        receptor 6)** using machine learning models trained on 
        bioactivity data from the **ChEMBL database**.
        
        #### 🎯 Target Information
        - **Target**: PfDHODH (Plasmodium falciparum dihydroorotate dehydrogenase)
        - **ChEMBL ID**: CHEMBL3486
        - **Organism**: Plasmodium falciparum
        - **Assay Type**: IC50 (half-maximal inhibitory concentration)
        - **Therapeutic Application**: Malaria disease
        ...
        #### 📊 Model Performance
        - **Ensemble R² Score**: > 0.74 (excellent predictive power)
        ...
        #### 👨‍💻 Development
        - **Pipeline**: Computational Drug Discovery Pipeline for Plasmodium falciparum
                        Inhibitors
        - **Institution**: CBIOS - Universidade Lusofona, Lisbon, Portugal for CADD 2.0  
                           Tutorial
        - **Date**: April, 2026
